...
class ThrowsKitchenSinkVisitor extends ASTVisitor {
	
	Path path;
	static int try_count;
	public ThrowsKitchenSinkVisitor(Path path) {
		this.path = path;
	}
			...
			if (block != null) {
				...
				for (int i = 0; i < statements.size(); i++) {
					if (statements.get(i).getNodeType() == Statement.IF_STATEMENT) {
						...
						ThrowsKitchenSinkVisitor.findThrows(elseStatement, throwDecSet);
						ThrowsKitchenSinkVisitor.findThrows(thenStatement, throwDecSet);
						...
						for (int j = 0; j < list.size(); j++) {
							ThrowsKitchenSinkVisitor.findThrows(list.get(j), throwDecSet);
						}
					}
				}
	...
	public static void findThrows(Statement statement, HashSet<String> throwDecSet) {
		if (statement != null) {
			if (statement.getNodeType() == Statement.VARIABLE_DECLARATION_STATEMENT) {
				...
				List<Statement> statements = block.statements();
				for (int i = 0; i < statements.size(); i++) {
					ThrowsKitchenSinkVisitor.findThrows(statements.get(i), throwDecSet);
				}
			}
		}
	}
